$(document).ready(function() {
    ...
    const privateKeyProperties = $('#ssh_key,#ssh_key_passphrase');
    const passwordProperties = $('#ssh_password');
    ...
    $("input:radio[name=ssh_method]").change(function() {

        const checked = $("input:radio[name=ssh_method]:checked").val();

        if (checked == 'private-key') {
            privateKeyProperties.show();
            passwordProperties.hide();
        } else {
            privateKeyProperties.hide();
            passwordProperties.show();
        }
    ...
    $(document).on('click', '#add_config', function() {
        if ($('#new_conf_conn_name').val() !== '' && $('#new_conf_conn_string').val() !== '') {
            ...
            var ssh_config = {
                enabled: $('#use_ssh').val(),
                username: $('#ssh_username').val(),
                address: $('#ssh_address').val(),
                port: $('#ssh_port').val(),
                auth_method: $('input[name=ssh_method]:checked').val(),
                password: $('#ssh_password').val(),
                key_path: $('#ssh_key').val(),
                key_passphrase: $('#ssh_key_passphrase').val()
            };
            ...
            data_obj[3] = JSON.stringify(ssh_config);
