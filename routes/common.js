...
exports.cleanSshOptions = function({ host, username, port, privateKey, password, passphrase }) {
    return JSON.stringify({
        ...
        passphrase: passphrase ? passphrase.replace(/./g, '*') : undefined,
    }, null, 4);
};

exports.initTunnel = function(config) {

    const clone = { ...config };

    if (clone.privateKey) {
        clone.privateKey = fs.readFileSync(config.privateKey);
    }
